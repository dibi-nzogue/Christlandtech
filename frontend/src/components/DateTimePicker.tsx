...
const ClockIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4 opacity-70">
    <path
      fill="currentColor"
      d="M12 1.75a10.25 10.25 0 1 0 0 20.5 10.25 10.25 0 0 0 0-20.5Zm.75 5a.75.75 0 0 0-1.5 0v5.19c0 .2.08.39.22.53l3.28 3.28a.75.75 0 1 0 1.06-1.06L12.75 12.2V6.75Z"
    />
  </svg>
);
...
const DateTimePicker: React.FC<Props> = ({
  ...
  const minutes = [...Array(Math.floor(60 / stepMinutes))].map(
    (_, i) => i * stepMinutes
  );
        ...
        <span className={value ? "text-gray-900" : "text-gray-400"}>
          {value
            ? new Intl.DateTimeFormat(undefined, {
                ...
                hour: "2-digit",
                minute: "2-digit",
              }).format(parseLocalDateTime(value) as Date)
            : placeholder}
        </span>
          ...
          >
            ...
            <div className="grid grid-cols-7 gap-1">
              {weeks.flat().map((d, i) => {
                ...
                const isSelected =
                  startOfDay(d).getTime() === selDay.getTime();
                const disabled =
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <select
                ...
                onChange={(e) =>
                  commit(selDay, Number(e.target.value), selected.getMinutes())
                }
              ...
              <select
                ...
                onChange={(e) =>
                  commit(selDay, selected.getHours(), Number(e.target.value))
                }
                className="border rounded-lg px-2 py-2 bg-white"
                aria-label="Minutes"
              >
