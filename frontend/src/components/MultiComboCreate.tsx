// src/components/MultiComboCreate.tsx
...
const MultiComboCreate: React.FC<Props> = ({
  ...
  const selectedIds = useMemo(
    () => new Set(value.map((v) => String(v.id ?? v.label))),
    [value]
  );
  ...
  const list = useMemo(() => {
    ...
    const base = options.filter(
      (o) => !selectedIds.has(String(o.id ?? o.label))
    );
    if (!q) return base;
  ...
  const remove = (opt: ComboOption) => {
    onChange(
      value.filter(
        (v) => (v.id ?? v.label) !== (opt.id ?? opt.label)
      )
    );
  };
  ...
  const exact = useMemo(
    () =>
      options.find(
        (o) =>
          o.label.toLowerCase() === query.toLowerCase().trim()
      ),
    [options, query]
  );
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "border rounded-lg p-2 bg-gray-100 w-full outline-[#00A9DC] flex flex-wrap gap-2",
          className
        )}
      >
        {value.map((v) => (
          <span
            key={`${v.id ?? v.label}`}
            className="px-2 py-1 rounded-md bg-white border text-sm flex items-center gap-1"
          >
            {v.label}
            <button
              type="button"
          ...
          {showCreate && (
            <>
              ...
              <button
                ...
                className="w-full text-left px-3 py-2 hover:bg-gray-100"
                onClick={() =>
                  add({ id: undefined, label: query.trim() })
                }
                role="option"
              >
                Créer « {query.trim()} »
